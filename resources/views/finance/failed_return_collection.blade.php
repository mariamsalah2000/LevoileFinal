        ...
        <section class="bg-white shadow-lg rounded-lg p-6 mb-8">
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-50 sticky top-0 z-10">
                        <tr>
                            <!-- Table Headers -->
                            @foreach (['#', 'Order Number','COD','Reason','Status','Action'] as $header)
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($details->where('status', 'failed') as $key => $success)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-center" id="options_{{$success->id}}">
                                    @if($trx->status != "closed" && $trx->status!="close" && $success->reason != "Order not Found" && $success->reason != "Order Not Found")
                                    <a
                                        class="bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600 transition duration-150"
                                        onclick="return confirm('Are you sure you want to Approve this Return?');" href="{{ route('returncollection.approveFailed',$success->id) }}">
                                        Approve
                                    </a>
                                    @endif
                                </td>
