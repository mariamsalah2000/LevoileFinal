                ...
                <div class="card">
                    <form action="" id="sort_orders" method="GET" class="p-5">
                        <div class="grid grid-cols-12 gap-4 mb-5">
                            ...
                            <div class="col-span-2">
                                <select class="form-select border-gray-300 rounded-md" name="status" id="branch_id">
                                    <option value="">Choose Status</option>
                                    <option value="open" @if ($status == 'open') selected @endif>Open</option>
                                    <option value="closed" @if ($status == 'closed') selected @endif>Converted</option>
                                </select>
                            </div>
                            <div class="col-auto">
                                        ...
                                        {{ $loop->iteration }}</td>
                                            ...
                                            <td class="text-center py-4 px-0 border border-gray-300">
                                                ...
                                                @if ($transaction->shipping_status == 'new')
                                                    <span
                                                        class="bg-blue-500 text-white px-2 py-1 rounded">{{ $transaction->shipping_status }}</span>
                                                @else
                                                <span
                                                        class="bg-green-300 text-white px-2 py-1 rounded">{{ $transaction->shipping_status }}</span>
                                                @endif
                                            </td>
                                                ...
                                                <div class="flex justify-center space-x-4">
                                                    ...
                                                    @if (auth()->user()->role_id != 8)
                                                        <div class="flex flex-col items-center space-y-1">
                                                            @if ($transaction->shipping_status == 'new')
                                                                <a class="bg-blue-500 text-white px-2 py-1 rounded" href="{{ route('returncollections.convert', $transaction->id) }}" title="Convert">
                                                    ...
                                                    @if ($transaction->shipping_status == 'new')
                                                        <div class="flex flex-col items-center space-y-1">
                                                            <a class="bg-red-500 text-white px-2 py-1 rounded" title="Delete" href="{{ route('returncollections.delete',$transaction->id) }}">
                                                                <i class="bi bi-x"></i>
                                                            </a>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $("#returncollections").addClass("active");
    </script>
@endsection
