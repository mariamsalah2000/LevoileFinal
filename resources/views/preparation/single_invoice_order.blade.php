    ...
    <div style="max-width:400px;margin:0 auto">
        ...
        <div class="centered">
            ...
            <h2>
                @if ($order->location_id)
                    {{ $order->location->name }} Branch Order
                @else
                    Online Order
                @endif
            </h2>
        ...
        @php
            ...
            echo ' <p style="text-align: center;display: block";font-size:20px>' .
                $barcode->getBarcodeSVG('Lvs' . $order->order_number, 'C128', 3, 37, 'Black', false) .
                '</p>';
        ...
        <p> Order Placing Date: {{ $order->created_at_date }}<br>
            ...
            Order Number : Lvs{{ $order->order_number }} <br>
        ...
        <table>
            ...
            <tbody>
                @foreach ($order_details as $key => $name)
                    @php
                        $qty += $name->order_qty;
                    @endphp
                    <tr>
                        <td colspan="2"> {{ $name->product_name }} X {{ $name->order_qty }} <br></td>
                        <td colspan="2" style="text-align: right;"> {{ $name->price }} <br></td>
                    </tr>
                @endforeach
