...
<body>
    ...
    <div style="max-width:400px;margin:0 auto">
        ...
        <p> Order Placing Date: {{ $order->created_at_date }}<br>
            ...
            Order Number : Lvs{{ $data['order_id'] }} <br>
            ...
            Address: {{ $order['shipping_address']['address1']??' ' }}<br>
            Cashier: {{ $auth_user }}
        </p>
        ...
        <table>
            ...
            <tbody>
                ...
                @foreach ($prepare->products as $key => $prod)
                    @if (!in_array($prod->product_id, $refunds->pluck('line_item_id')->toArray()))
                        <tr>
                            <td colspan="2"> {{ $prod->product_name }} X {{ $prod['order_qty'] }} <br></td>
                    ...
                    @else
                        @php
                            $refund = $refunds->where('line_item_id',$prod->product_id)->first();
                        @endphp

                        @if($refund->quantity < $prod['order_qty'])
                            <tr>
                                <td colspan="2"> {{ $prod->product_name }} X {{ $prod['order_qty'] - $refund->quantity }} <br></td>
                                <td colspan="2" style="text-align: right;">
                                    {{ $prod->price }} <br>
                                </td>
                            </tr>
            ...
            <tfoot>
                ...
                <tr style="border-top: 2px solid #000 !important;border-bottom: 2px solid #000 !important;">
                    ...
                    <th style="text-align:right">{{ $order->subtotal_price }}</th>
                </tr>
                <tr style="border-top: 2px solid #000 !important;border-bottom: 2px solid #000 !important;">
                ...
                <tr style="border-top: 2px solid #000 !important;border-bottom: 2px solid #000 !important;">
                    ...
                    <th style="text-align:right">
                        {{ is_array($order->payment_gateway_names) && isset($order->payment_gateway_names[0]) ? $order->payment_gateway_names[0] : '' }}
                    </th>
                </tr>
                <tr style="border-top: 2px solid #000 !important;border-bottom: 2px solid #000 !important;">
                    <th colspan="2">Payment Status</th>
                    <th style="text-align:right">{{ $order->getPaymentStatus() }}</th>
                </tr>
    ...
    <script type="text/javascript">
        function auto_print() {
            window.print();
            window.location.href = '{{ route('prepare.all', ['delivery_status' => 'prepared']) }}';
        }
